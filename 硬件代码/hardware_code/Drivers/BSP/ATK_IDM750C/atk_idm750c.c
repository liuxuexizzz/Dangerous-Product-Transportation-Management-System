...
#include "./BSP/ATK_IDM750C/atk_idm750c_cfg.h"
...
uint8_t atk_idm750c_http_request_mode(void)  // 注意，移除了参数
{
    uint8_t ret;
    char cmd[50];
...
uint8_t atk_idm750c_http_url_address(char *http_address)
{
   ...
   sprintf(cmd, "AT+HTTPURL=\"http://1270778.js.cn:8080/state\"");
...
uint8_t atk_idm750c_http_request_header_information(void)
{
    ...
    // 设置HTTP请求头为 "Content-Type:application/json"
    sprintf(cmd, "AT+HTTPHD=\"Content-Type:application/json\"");
    printf("222");
    ret = atk_idm750c_send_cmd_to_dtu(cmd, "OK", ATK_IDM750C_TIME);
